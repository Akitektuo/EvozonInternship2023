...
namespace MealPlan.UnitTests.Business.Services.Orders
{
    [TestFixture]
    public class UpdateOrderServiceTests
    {
        ...
        public void Init()
        {
            ...
            _service = new OrderStatusService(_context.Object);
        ...
        [Test]
        public async Task WhenApproveValidOrderId_ShouldReturnTrue()
        {
            _request = await _context.Object.Orders.Where(o => o.StatusId == Status.WaitingForApproval).Select(o => o.Id).FirstOrDefaultAsync();

            var result = await _service.ApproveOrder(_request);

            result.Should().BeTrue();
        }

        [Test]
        public async Task WhenApproveInvalidOrderId_ShouldThrowError()
        {
            _request = 0;

            var action = async () => await _service.ApproveOrder(_request);

            await action.Should().ThrowAsync<CustomApplicationException>().Where(e => e.ErrorCode == ErrorCode.OrderDoesNotExist);
        }

        [Test]
        public async Task WhenApproveValidOrderIdAlreadyApproved_ShouldThrowError()
        {
            _request = await _context.Object.Orders.Where(o => o.StatusId == Status.Approved).Select(o => o.Id).FirstOrDefaultAsync();
        ...
        [Test]
        public async Task WhenRejectValidOrderId_ShouldReturnTrue()
        {
            _request = await _context.Object.Orders.Where(o => o.StatusId == Status.WaitingForApproval).Select(o => o.Id).FirstOrDefaultAsync();

            var result = await _service.RejectOrder(_request);

            result.Should().BeTrue();
        }

        [Test]
        public async Task WhenRejectInvalidOrderId_ShouldThrowError()
        {
            _request = 0;

            var action = async () => await _service.RejectOrder(_request);

            await action.Should().ThrowAsync<CustomApplicationException>().Where(e => e.ErrorCode == ErrorCode.OrderDoesNotExist);
        }

        [Test]
        public async Task WhenRejectValidOrderIdAlreadyRejected_ShouldThrowError()
        {
            _request = await _context.Object.Orders.Where(o => o.StatusId == Status.Rejected).Select(o => o.Id).FirstOrDefaultAsync();
        ...
        [Test]
        public async Task WhenApproveValidOrderIdValidStatusButOutdatedOrder_ShouldThrowError()
        {
            _request = await _context.Object.Orders.Where(o => (o.StatusId == Status.WaitingForApproval || o.StatusId == Status.Rejected) && o.StartDate < DateTime.UtcNow).Select(o => o.Id).FirstOrDefaultAsync();

            var action = async () => await _service.ApproveOrder(_request);

            await action.Should().ThrowAsync<CustomApplicationException>().Where(e => e.ErrorCode == ErrorCode.OrderIsOutdated);
        }
        ...
        private void SetupContext()
        {
            ...
            var orders = new List<Order>
            {
                ...
                new Order { Id = 4, Menu = menus[2], MenuName = menus[2].Name, MenuId = menus[2].Id, User = users[0], UserId = users[0].Id, UserEmail = users[0].Email, StartDate = DateTime.UtcNow.AddDays(-1), EndDate = DateTime.UtcNow.AddDays(20), PhoneNumber = "fffdas", ShippingAddress = "fsdafdas", StatusId = Status.WaitingForApproval, TotalPrice = 100},
            };
